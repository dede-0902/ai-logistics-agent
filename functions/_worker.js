  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/api/log' && method === 'POST') {
        ...
        logs.unshift({ id: ++logId, timestamp: b.timestamp||new Date().toISOString(), sessionId: b.sessionId||'', conversationId: b.conversationId||'', userInput: (b.userInput||'').slice(0,500), aiResponse: (b.aiResponse||'').slice(0,2000), latencyMs: b.latencyMs??0, tokens: b.tokens??0, hasData: b.hasData??false, isError: b.isError??false, errorMessage: b.errorMessage||'' })
        if (logs.length > 2000) logs.length = 2000
        return new Response(JSON.stringify({ ok: true }), { headers: { 'content-type': 'application/json' } })
      }
      if (path === '/api/coze' && method === 'POST') {
        const token = env.COZE_TOKEN
        if (!token) return new Response(JSON.stringify({ error: 'COZE_TOKEN not set' }), { status: 500, headers: { 'content-type': 'application/json' } })
        const { input, conversationName } = await request.json()
        const r = await fetch('https://api.coze.cn/v1/workflows/chat', { method: 'POST', headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' }, body: JSON.stringify({ workflow_id: '7663285609365225499', additional_messages: [{ role: 'user', content_type: 'text', content: input }], parameters: { CONVERSATION_NAME: conversationName || 'web_' + Date.now() } }) })
        return new Response(await r.text(), { headers: { 'content-type': 'text/event-stream' } })
      }
      if (path === '/api/logs') return new Response(JSON.stringify({ logs: logs.slice(0,100), total: logs.length }), { headers: { 'content-type': 'application/json' } })
      if (path === '/api/stats') {
