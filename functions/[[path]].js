/**
 * Cloudflare Pages Function — 处理 API 和 Admin 路由
 * 静态文件由 Pages 自动托管
 */
const logs = []
let logId = 0
...
export async function onRequest(context) {
  ...
  // 非 API/Admin 路由 → 交给 Pages 托管静态文件
  ...
  try {
    ...
    if (pathname === '/api/logs') {
      return json({ logs: logs.slice(0, 100), total: logs.length })
    }

    // GET /api/stats
    if (pathname === '/api/stats') {
      const total = logs.length, errors = logs.filter(l => l.isError).length
      ...
      logs.forEach(l => {
        if (l.userInput) {
          l.userInput.replace(/[^一-龥a-zA-Z0-9]/g, ' ').split(/\s+/).filter(Boolean).forEach(w => {
            if (w.length >= 2) wc[w] = (wc[w] || 0) + 1
          })
        }
      })
      return json({
...
function ko() {
  return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>看板</title><style>
